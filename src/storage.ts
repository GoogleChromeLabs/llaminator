...
// kCurrentBlobIDKey is the key into the miscStringVals store for the
// FileUniqueID of the "current" blob (ie, the one that will be displayed
// when the app is first opened).
const kCurrentBlobIDKey = 'currentBlobID';
...
export class LlamaStorage {
  ...
  async add(file: Blob, metadata: FileMetadata): Promise<FileRecord> {
    ...
    await Promise.allSettled([
      ...
      this.db.put('miscStringVals', id, kCurrentBlobIDKey),
    ]);
  ...
  async getCurrentID(): Promise<FileUniqueID | undefined> {
    const id = await this.db.get('miscStringVals', kCurrentBlobIDKey);
    console.log(`current blob ID: ${id}`);
    return id;
  }
