...
const firebaseDir = 'public';
const remoteIdPrefix = 'R-';
...
export class LlamaStorage {
  ...
  async add(file: Blob, metadata: FileMetadata): Promise<FileRecord> {
    ...
    if (firebaseStorage) {
      ...
      const storageRef = firebase.ref(firebaseStorage, firebaseDir + '/' + id);
  ...
  async list(): Promise<FileRecord[]> {
    ...
    if (firebaseStorage) {
      firebaseList = await firebase.listAll(firebase.ref(firebaseStorage, firebaseDir));
    }
  ...
  private async getRemoteFile(remoteId: FileUniqueID): Promise<Blob | undefined> {
    if (!firebaseStorage || remoteId.indexOf(remoteIdPrefix) !== 0) {
      return undefined;
    }
    remoteId = remoteId.slice(remoteIdPrefix.length); // Trim the 'R-' prefix.

    // TODO: save a copy locally
    return firebase.getBlob(firebase.ref(firebaseStorage, firebaseDir + '/' + remoteId),
  ...
  private async getRemote(remoteId: FileUniqueID): Promise<FileRecord | undefined> {
    ...
    return this.getRemoteFirebaseMetadata(
        firebase.ref(firebaseStorage, firebaseDir + '/' + remoteId));
  }
